...
public class ResultServiceImpl extends BaseServiceImpl<ResultDto, Result, String> implements ResultService {
	...
	private Map<String, Double> option2Data(ResultDto resultDto) throws Exception {
		...
		for (RiskDto riskDto : riskDtoList) {
			...
			switch (riskName) {
			case "FamilyHistory":
				riskValue = resultDto.getQ0() <= 1 ? (resultDto.getQ0() < 1 ? 0 : 2.2) : 4.0;
				break;
			case "InflammatoryBowelDisease":
				riskValue = resultDto.getQ1() == 1 ? 1.7 : 0;
				break;
			case "Diabetes":
				riskValue = resultDto.getQ2() == 1 ? 1.3 : 0;
				break;
			case "AlcoholConsumption":
				riskValue = resultDto.getQ3() == 0 ? 0 : (resultDto.getQ3() == 1 ? 1.2 : 1.4);
				break;
			case "Obesity":
				riskValue = resultDto.getQ4() == 1 ? 1.3 : 0;
				break;
			case "RedMeatConsumption":
				riskValue = resultDto.getQ5() == 1 ? 1.2 : 0;
				break;
			case "ProcessedMeatConsumption":
				riskValue = resultDto.getQ6() == 1 ? 1.2 : 0;
				break;
			case "Smoking":
				riskValue = resultDto.getQ7() == 1 ? 1.2 : 0;
				break;
			case "PhysicalActivity":
				riskValue = resultDto.getQ8() == 1 ? -0.7 : 0;
				break;
			case "DairyConsumption":
				riskValue = resultDto.getQ9() == 1 ? -0.8 : 0;
				break;
			case "MilkConsumption":
				riskValue = resultDto.getQ10() == 1 ? -0.9 : 0;
				break;
			default:
				riskValue = 0.0;
				break;
			}
			riskData.put(riskName, NumberUtil.reserveTwo(riskValue*10/13.3));
		}
